...
CFG = {
    ...
    "pseudo_rounds":          3,     # iterations after round 0
    "pseudo_threshold":       0.85,  # score >= thr → pseudo pos; score <= 1-thr → pseudo neg
    "max_pseudo_per_class":   {0: 1000, 1:1600,2: 2000},  # cap per class per round: round0→1000, round1→1600
...
def self_training(train_df: pd.DataFrame, test_df: pd.DataFrame,
    ...
    for rnd in range(total_rounds):
        ...
        if not is_last:
            ...
            _mpc_cfg         = CFG.get("max_pseudo_per_class", None)
            max_per_class    = _mpc_cfg.get(rnd) if isinstance(_mpc_cfg, dict) else _mpc_cfg
...
# ── Metrics helper ────────────────────────────────────────────────────────
def _log_metrics(tag: str, y_true, y_score, threshold: float = 0.5) -> dict:
...
def main():
    ...
    log("=" * 60)
    log(f"  mxbai SimCSE + Pseudo-labeling Pipeline ({n_folds}-Fold CV)")
    ...
    # Stage 1: SimCSE once on all texts (shared across folds)
    ...
    # Stage 2+3: 5-Fold CV — each fold runs the full self-training pipeline
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, train_df["LABEL"])):
        ...
        log(f"  fold_train={len(fold_train)}  fold_val={len(fold_val)}"
            f"  (pos={fold_train['LABEL'].sum()}, neg={(fold_train['LABEL']==0).sum()})")

        pipe, sbert_fold = self_training(fold_train, test_df, sbert_base=sbert)

        # OOF predictions on held-out fold
        val_emb  = embed(fold_val, sbert_fold, has_label=True)
        fold_oof = np.clip(pipe.predict(val_emb[FEAT_COLS].values), 0, 1)
        oof_scores[val_idx] = fold_oof
        _log_metrics(f"Fold {fold + 1}", fold_val["LABEL"].values, fold_oof)
        del val_emb, fold_oof

        # Test predictions for this fold
        test_emb  = embed(test_df, sbert_fold, has_label=False)
        fold_test = np.clip(pipe.predict(test_emb[FEAT_COLS].values), 0, 1)
        test_scores_list.append(fold_test)
        del test_emb, fold_test, pipe, sbert_fold
        _free_memory()

    log(f"\n── OOF Summary ({n_folds} folds) ──")
    _log_metrics("OOF all", train_df["LABEL"].values, oof_scores)

    # # Average test predictions across folds
    # test_scores = np.mean(test_scores_list, axis=0)
    # test_out = test_df[["row_id"]].copy()
    # test_out["LABEL"] = (test_scores >= 0.5).astype(int)
