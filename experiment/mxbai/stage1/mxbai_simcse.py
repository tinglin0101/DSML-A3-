...
CFG = {
    ...
    "test_output": base_path  + "_result.csv",
    ...
    # Stage 2: supervised fine-tune on labeled train pairs
    "ft_epochs":   3,
    "ft_lr":       2e-5,
    "ft_batch":    16,
    "ft_pairs":    3000,
...
def finetune_simcse(sbert: SentenceTransformer, all_texts: list) -> None:
    """
    Unsupervised SimCSE: each text is its own positive pair.
    The encoder's dropout produces two different representations of the same text;
    all other texts in the batch serve as negatives via MultipleNegativesRankingLoss.
    No labels required — uses both train and test texts.
    """
...
def finetune_supervised(sbert: SentenceTransformer, train_df: pd.DataFrame,
    ...
    log(f"  [Stage 2] Supervised fine-tune"
        f"  pairs={CFG['ft_pairs']}  epochs={CFG['ft_epochs']}"
...
# ── Two-stage training ────────────────────────────────────────────────────────
def train_two_stage(train_df: pd.DataFrame, test_df: pd.DataFrame,
    ...
    log(f"  Total texts for SimCSE: {len(all_texts)}"
        f"  (train={len(train_df)}, test={len(test_df)})")

    sbert = copy.deepcopy(sbert_base)

    # Stage 1: unsupervised SimCSE on all texts (no labels used)
    finetune_simcse(sbert, all_texts)

    # Stage 2: supervised fine-tune on labeled train pairs only
    finetune_supervised(sbert, train_df, output_path=CFG["ft_save"])

    X_full = embed(train_df, sbert, has_label=True)[FEAT_COLS].values
...
def nested_cv(train_df: pd.DataFrame, test_df: pd.DataFrame,
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(np.arange(len(train_df)), y_all)):
        ...
        # Stage 1: SimCSE on fold-train texts + all test texts (no labels used)
        all_texts = fold_train["TEXT"].astype(str).tolist() + test_texts
        log(f"  total SimCSE texts={len(all_texts)}"
            f"  (fold_train={len(fold_train)}, test={len(test_texts)})")
        finetune_simcse(sbert, all_texts)

        # Stage 2: supervised fine-tune on fold-train labels only
        finetune_supervised(sbert, fold_train, output_path=None)

        # Embed fold-train and val with this fold's SBERT (val never seen during training)
...
def main():
    global _log_fh
    CFG["test_output"] = base_path + "_result.csv"
    # CFG["ft_save"]     = base_path +"_ft_model"
    _log_fh = open("log.txt", "w", encoding="utf-8")

    log("=" * 60)
    log(f"  mxbai SimCSE Two-stage Pipeline")
    ...
    pipe, sbert_final = train_two_stage(train_df, test_df, sbert)
