# =============================================================
# vBERT_small — 使用 Training Set 微調 (Fine-tune) DistilBERT 進行情感分類
#
# 策略：
#   將 train_2022.csv 以 8:2 比例切分：
#     - 80% (train_split)：用來微調 distilbert-base-uncased 的分類頭
#     - 20% (val_split)  ：用來驗證微調效果（Validation Set）
#
# 模型架構：
#   distilbert-base-uncased + Linear Classification Head
#   （DistilBertForSequenceClassification）
#   DistilBERT 是 BERT 的輕量版（6 層），速度約快 60%，不使用 token_type_ids
#
# 輸出：
#   - vBERT_small.csv：對 test_no_answer_2022.csv 的預測結果
...
class TestDataset(Dataset):
    """測試集（無標籤）專用 Dataset。DistilBERT 無 token_type_ids。"""
...
# DistilBertForSequenceClassification = distilbert-base-uncased + Linear(hidden=768, num_labels=2)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), os.path.join(script_dir, "best_vBERT_small.pt"))
...
model.load_state_dict(torch.load(os.path.join(script_dir, "best_vBERT_small.pt"), map_location=device))
