...
def apply_pseudo_labels(
    ...
    """
    source='test'       — augment train with high-conf test pseudo-labels (original).
    source='train+test' — replace low-conf train labels + add high-conf test pseudo-labels.
    """
    # ── test side ────────────────────────────────────────────────────────────
    test_mask        = (test_scores <= thresh_low) | (test_scores >= thresh_high)
    test_pseudo_lbl  = (test_scores >= thresh_high).astype(int)
    pseudo_test_df          = test_df[test_mask].copy().reset_index(drop=True)
    pseudo_test_df["LABEL"] = test_pseudo_lbl[test_mask]

    n_test  = int(test_mask.sum())
    n_pos_t = int(test_pseudo_lbl[test_mask].sum())
    log(f"  Pseudo-label filter: score ≤ {thresh_low} or ≥ {thresh_high}")
    log(f"    [test]  Selected {n_test}/{len(test_df)} samples  (pos={n_pos_t}, neg={n_test-n_pos_t})")

    # ── train side (only when source='train+test') ────────────────────────
    if source == "train+test" and train_scores is not None:
        tr_mask       = (train_scores <= thresh_low) | (train_scores >= thresh_high)
        tr_pseudo_lbl = (train_scores >= thresh_high).astype(int)
        # replace labels for high-confidence train rows; keep others as-is
        base_train_df          = train_df.copy()
        base_train_df.loc[tr_mask, "LABEL"] = tr_pseudo_lbl[tr_mask]
        n_tr  = int(tr_mask.sum())
        n_pos_r = int(tr_pseudo_lbl[tr_mask].sum())
        log(f"    [train] Re-labeled {n_tr}/{len(train_df)} samples  (pos={n_pos_r}, neg={n_tr-n_pos_r})")
    else:
        base_train_df = train_df

    augmented = pd.concat([base_train_df, pseudo_test_df], ignore_index=True)
    n_added   = n_test
...
CFG = {
    ...
    "test_output":  base_path + "result_mpnet_c.csv",
    ...
    "da_lr":        2e-5,
    "da_mlm_prob":  0.15,
    "da_save":      base_path + "mpnet_c_da_model.pt",
    ...
    "ft_pairs":     3000,
    "ft_warmup":    100,
    "ft_save":      base_path + "mpnet_c_ft_model.pt",

    # C: iterative pseudo-label (repeat A+B multiple rounds)
    "pseudo_iters":       3,
    "pseudo_thresh_low":  0.15,
    "pseudo_thresh_high": 0.85,
    # "test" = augment with test pseudo-labels only (original)
    # "train+test" = also re-label high-conf train rows each iteration
    "pseudo_source":      "test",
}
...
def main():
    ...
    sep = "=" * 60
    log(f"\n{sep}")
    log("  mpnet_c — Pseudo-Label C (iterative)")
    log("  CV → [predict test → high-conf → retrain SBERT + classifier] x N")
    log(sep)
    log(f"  Input      : {c['input']}")
    log(f"  Test       : {c['test']}")
    log(f"  Model      : {c['model']}")
    ...
    log(f"  Pseudo-L   : C  iters={c['pseudo_iters']}  thresh=[≤{c['pseudo_thresh_low']}, ≥{c['pseudo_thresh_high']}]  source={c['pseudo_source']}\n")
    ...
    # train final_scores for pseudo_source='train+test'
    train_final_scores = row_scores["final_score"].values

    # ── C: iterative pseudo-label ─────────────────────────────────────────────
    log(f"\n── Pseudo-Label C  (iters={c['pseudo_iters']}, "
        f"thresh=[≤{c['pseudo_thresh_low']}, ≥{c['pseudo_thresh_high']}], "
        f"source={c['pseudo_source']}) ──")
    for it in range(1, c["pseudo_iters"] + 1):
        log(f"\n  ══ Iteration {it}/{c['pseudo_iters']} ══")
        augmented_df, n_added = apply_pseudo_labels(
            train_df, test_df, test_scores,
            c["pseudo_thresh_low"], c["pseudo_thresh_high"],
            source=c["pseudo_source"],
            train_scores=train_final_scores,
        )
        if n_added == 0:
...
if __name__ == "__main__":
    import argparse, ast
    parser = argparse.ArgumentParser(description="mpnet C iterative pseudo-label")
    parser.add_argument("--set", nargs="*", metavar="KEY=VALUE",
                        help="Override CFG keys, e.g. --set pseudo_iters=5 pseudo_thresh_low=0.1")
