"""
End-to-End Sentiment Scoring Pipeline  —  v2
=============================================
v2 change: Steps 1–2 are now processed row-by-row.
  For each row:
    Step 1 — split text at transition words (Stanza)
    Step 2 — compute NRC emotion scores per segment
    Aggregate — Sum( emotion_count_i * weight(i) )  where weight(i) = sqrt(i+1)
  After all rows are aggregated, Step 3 — regression on the aggregated features.

Usage:
    python pipeline_v2.py --input dataset_split/train_2022.csv --output row_scores.csv
    python pipeline_v2.py --input dataset_split/train_2022.csv --model ridge
"""
...
# Aggregated feature columns (count + freq, weighted-summed across segments)
AGG_FEATURES = (
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 1 — Text splitting  (unchanged from v1)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 2 — NRC emotion scores (single segment)
# ══════════════════════════════════════════════════════════════════════════════

def _get_emotion_scores(nrc: NRCLex, text: str) -> dict:
    """Return raw emotion counts and frequencies for one segment."""
    nrc.load_raw_text(text)
    raw  = nrc.raw_emotion_scores
    freq = nrc.affect_frequencies
    result = {}
    for e in EMOTIONS:
        result[EMOTION_COUNT_COLS[e]] = raw.get(e, 0)
        result[EMOTION_FREQ_COLS[e]]  = round(freq.get(e, 0.0), 6)
    return result
...
def process_row(text: str, label: int, row_id: int,
                nlp: stanza.Pipeline, nrc: NRCLex) -> dict:
    """
    Run Step 1 (split) + Step 2 (emotion) on a single row, then aggregate:
        agg_feature = Sum_i( feature_i * weight(i) )   weight(i) = sqrt(i + 1)
    Applied to both count and freq features (16 total).

    Returns a flat dict:  row_id, LABEL, Anger_count, Anger_freq, …
    """
    # Step 1 — split
    ...
    for i, seg in enumerate(segments):
        counts = _get_emotion_scores(nrc, seg)
        for col in AGG_FEATURES:
            agg[col] += counts[col] * weights[i]
...
def run_row_by_row(df: pd.DataFrame, nlp: stanza.Pipeline,
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        records.append(
            process_row(str(row["TEXT"]), label, int(row["row_id"]), nlp, nrc)
        )
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 3 — Regression scoring  (same as v1, adapted for flat feature table)
# ══════════════════════════════════════════════════════════════════════════════
...
def predict_test(df_test: pd.DataFrame, nlp: stanza.Pipeline,
    ...
    """
    agg_df     = run_row_by_row(df_test, nlp, has_label=False)
...
def main():
    parser = argparse.ArgumentParser(description="End-to-end sentiment scoring pipeline v2")
    ...
    sep = "=" * 60
    print(f"\n{sep}")
    print("  End-to-End Sentiment Scoring Pipeline  [v2]")
    print(sep)
    print(f"  Input : {args.input}")
    print(f"  Output: {args.output}")
    print(f"  Model : {args.model}")
    print(f"  Aggregation: Sum( emotion_count_i * sqrt(i+1) )\n")
    ...
    # ── Steps 1+2: row-by-row split → emotion → weighted sum ─────────────────
    print("── Steps 1+2: Split + Emotion (row-by-row) ─────────────────")
    nlp    = _build_stanza_pipeline()
    agg_df = run_row_by_row(df, nlp, has_label=True)
    ...
    #     all_batches = []
        ...
        #     all_batches.append(predict_test(batch_df, nlp, trained_pipe))
