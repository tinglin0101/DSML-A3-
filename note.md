...
"da_epochs":   1       # 可增加到 2-3，讓 encoder 更適應 domain
"da_batch":    8       # 顯存夠可調大（16/32）
"da_lr":       3e-5    # 學習率，一般不需大動
"da_mlm_prob": 0.15    # 遮蔽比例，可試 0.10~0.20
2. Phase 2 — Supervised Fine-tuning


"ft_epochs":  3        # 可增加，但小心 overfitting
"ft_lr":      2e-5     # 學習率
"ft_batch":   16
"ft_pairs":   3000     # pair 數量，可增加到 5000-10000
"ft_warmup":  100      # warmup steps
3. ML 分類模型
在 baseline.ipynb 的 _make_model 函式中：


"model": "rf"   # 可換成 "gbm" 或 "elastic"
各模型的超參數也可以調：
...
ft_pairs 從 3000 → 5000（最低成本，通常有效果）
"model" 換成 "gbm"（GBM 通常比 RF 準）
ft_epochs 從 3 → 5